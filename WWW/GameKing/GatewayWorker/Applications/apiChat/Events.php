...
class Events
{
    ...
    {
        self::$db = new \GatewayWorker\Lib\DbConnection('127.0.0.1', '3306', 'yingtao', 'yingtao', 'yingtao');
    }
    ...
    public static function onMessage($client_id, $message)
    {
        ...
        switch ($message_data['type']) {
            case 'bind':
                ...
                Gateway::sendToUid($message_data['uid'], json_encode(['type'=>'bind','msg'=>'绑定成功']));
                return;
            case 'bindRead':
                self::$db->update('kg_chat_message')->cols(['readtime' => time()+8*3600])
            ...
            case 'say':
                ...
                //获取发送者历史记录
                $re = self::$db->select('*')->from('kg_chat_history')->where('uid= :uid and other_uid=:to_uid')->bindValues(['uid' => $message_data['from_uid'], 'to_uid' => $message_data['to_uid']])->row();
                if (empty($re)) {
                    self::$db->insert('kg_chat_history')->cols([
                        'uid' => $message_data['from_uid'],
                        'other_uid' => $message_data['to_uid'],
                        'addtime' => time()+8*3600,
                    ])->query();
                } else {
                    self::$db->update('kg_chat_history')->cols(['addtime' => time()+8*3600])
                        ->where('uid= :uid and other_uid=:to_uid')->bindValues(['uid' => $message_data['from_uid'], 'to_uid' => $message_data['to_uid']])
                        ->query();
                }
                //获取接受者历史记录
                $res = self::$db->select('*')->from('kg_chat_history')->where('uid= :uid and other_uid=:to_uid')->bindValues(['uid' => $message_data['to_uid'], 'to_uid' => $message_data['from_uid']])->row();
                if (empty($res)) {
                    self::$db->insert('kg_chat_history')->cols([
                ...
                //获取上一条记录的发送时间
                $lastTime = self::$db->select('sendtime')->from('kg_chat_message')
                ...
                $insert_id = self::$db->insert('kg_chat_message')->cols([
                    'from_uid' => $message_data['from_uid'],
                    'to_uid' => $message_data['to_uid'],
