...
class Base extends Controller
{
    ...
    public function _initialize()
    {
        ...
        $admin = db("admin")->field("id,uid,name,cat,status")->where("id=" . $adminid)->find();
        if (!$admin) {
            cookie("adminid", NULL);
            $this->error('login/index');
        }
...
//			}
        ...
        /*if($admin['cat'] == 1){
            ...
            $vip = db('member')->where('uid',$admin['uid'])->field('vip,vip_from,vip_end')->find();
            $company_name = db('renzheng')->where('adminid',$admin['id'])->value('company_name');
            $this->assign("vip", $vip);
        }
