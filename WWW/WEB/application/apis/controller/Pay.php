...
{   
    //支付接口
    function pay(){
        $pay_type=input('pay_type');    //1.支付宝 2.微信 3余额
        $id=input('id');
        // $id='QXG1552874551205';
        $type=1;    //1.order_id    2.order_sn
        if(strpos($id,'QXG') !== false){ 
            //存在order_dn
            $type=2;
        }else{
           $type=1;
        ...
        if($pay_type==1){
            $data=$this->pay_alipay($type,$id);
            return json(['code'=>200,'msg'=>'支付成功','data'=>$data]);
        }elseif($pay_type==2){
            $data=$this->wxpay($type,$id);
            return json(['code'=>200,'msg'=>'支付成功','data'=>$data]);
        }else{
            if($type==1){
                    //order_id
                    $find=db('order')->where('id',$id)->field('price,uid')->find();
                    $user=db('user')->where('id',$find['uid'])->field('money')->find();
                    if($find['price']>$user['money']){
                        return json(['code'=>101,'msg'=>'余额不足','data'=>'']);
                    }
                    $res=db('order')->where('id',$id)->update(['status'=>2]);
                    if($res){
                        $new_money=$user['money']-$find['price'];
                        $new_money=sprintf('%.2f',$new_money);
                        db('user')->where('id',$find['uid'])->update(['money'=>$new_money]);
                        return json(['code'=>200,'msg'=>'支付成功','data'=>'']);
                    }else{
                        return json(['code'=>100,'msg'=>'支付失败','data'=>'']);
                    }
            }else{
                 //存在order_sn
                 $find_price=db('order')->where('order_sn',$id)->sum('price');
                 $find=db('order')->where('order_sn',$id)->field('uid')->find();
                 $user=db('user')->where('id',$find['uid'])->field('money')->find();
                 if($find_price>$user['money']){
                    return json(['code'=>101,'msg'=>'余额不足','data'=>'']);
                }
                $res=db('order')->where('order_sn',$id)->update(['status'=>2]);
                if($res){
                    $new_money=$user['money']-$find_price;
                    $new_money=sprintf('%.2f',$new_money);
                    db('user')->where('id',$find['uid'])->update(['money'=>$new_money]);
                    return json(['code'=>200,'msg'=>'支付成功','data'=>'']);
                }else{
     ...
     */
    //支付宝支付
    function  pay_alipay($type,$id)
    {
        require_once EXTEND_PATH . 'Alipay/AopEncrypt.php';
        //创建订单
        if($type==2){
            //order_sn
            $find = db('order')->where('order_sn',$id)->find();
        }else{
            //order_id
            $find = db('order')->where('id',$id)->find();
        }
        if(!$find){
            return json(['code'=>101,'msg'=>'订单未找到','data'=>'']);
        }
        $money = 0.01;
        //发起支付
        $c = new AopClient;
        ...
        $content['out_trade_no'] = $id;//商户网站唯一订单号
