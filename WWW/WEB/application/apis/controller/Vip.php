...
{   
    ...
    function pay(){
        ...
        $price=input('price');
        if($pay_type==1){
            $data=$this->pay_alipay($uid,$price);
            return json(['code'=>200,'msg'=>'开通成功','data'=>$data]);
        }elseif($pay_type==2){
            $data=$this->wxpay($uid,$price);
            return json(['code'=>200,'msg'=>'开通成功','data'=>$data]);
        }else{
            $user=db('user')->where('id',$uid)->field('money,is_vip,end_time')->find();
            //is_vip 1不是2：是
            if($user['money']<$price){
            ...
            if($user['is_vip']==1){
                ...
                $res=db('user')->where('id',$uid)->update(['is_vip'=>2,'end_time'=>$year,'money'=>$new_money]);
                if($res){
                    return json(['code'=>200,'msg'=>'vip开通成功','data'=>'']);
                }else{
                    return json(['code'=>100,'msg'=>'开通失败','data'=>'']);
                }
            }else{
                $new_time=strtotime("+1 years 1months",$user['end_time']);
                $new_money=$user['money']-$price;
                $res=db('user')->where('id',$uid)->update(['end_time'=>$new_time,'money'=>$new_money]);
                if($res){
                    return json(['code'=>200,'msg'=>'vip开通成功','data'=>'']);
                }else{
                    return json(['code'=>100,'msg'=>'开通失败','data'=>'']);
    ...
    public function alipay_notify_url()
    {
        ...
        if ($_POST['trade_status'] == 'TRADE_FINISHED') {
            ...
            $uid=$out_trade_no;
            $user=db('user')->where('id',$uid)->field('is_vip,end_time')->find();
            if($user['is_vip']==1){
                $year=strtotime("+1 year");
                $res=db('user')->where('id',$uid)->update(['is_vip'=>2,'end_time'=>$year]);
            }else{
                $new_time=strtotime("+1 years 1months",$user['end_time']);
                $res=db('user')->where('id',$uid)->update(['end_time'=>$new_time]);
            }
        }
