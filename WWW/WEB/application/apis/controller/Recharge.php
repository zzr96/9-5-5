...
{   
    ...
    function  pay_alipay($price,$uid)
    {
        ...
        $param['notify_url'] = 'http://liumai.niurenjianzhan.com/apis/recharge/alipay_notify_url';//支付宝服务器主动通知地址
    ...
    public function alipay_notify_url()
    {
        ...
        if ($_POST['trade_status'] == 'TRADE_FINISHED'){
           ...
           //成功回调
           $find=db('user')->where('id',$uid)->field('money')->find();
           db('user')->where('id',$uid)->update(['money'=>$find['money']]);
        }
     ...
     * param: order_id - {int} 订单号 必须
     */
    public function wxpay($price,$uid)
    {   
        $id='123456789'.$uid;
        // $price = 0.01;
        //发起支付
        ...
        $parameter = array(
            ...
            'notify_url' => 'http://quxiguan.tainongnongzi.com/public_html/index.php/apis/recharge/weixin_notify_url',//String(256)  接收微信支付异步通知回调地址，通知url必须为直接可访问的url，不能携带参数。
            'trade_type' => 'APP'//支付类型
        );
