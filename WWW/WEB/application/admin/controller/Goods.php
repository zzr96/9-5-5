...
class Goods extends Base
{
    ...
    function cate(){
        ...
        $adminid = cookie("adminid");
        if($adminid==1){
            $list = db("goods_cate")->where(['fid'=>0,'shop_id'=>1])->order("dateline asc")->select();
            foreach ($list as $k => $v){
                $list[$k]['item'] = db("goods_cate")->where(["fid"=>$v['id'],'shop_id'=>1])->order("dateline asc")->select();
            }
        }else{
            $list = db("goods_cate")->where(['fid'=>0,'shop_id'=>$shop_id])->order("dateline asc")->select();
            foreach ($list as $k => $v){
                $list[$k]['item'] = db("goods_cate")->where(["fid"=>$v['id'],"shop_id"=>$shop_id])->order("dateline asc")->select();
            }
        }
        $this->assign("list", $list);
    	return view();
    ...
    function cate_add_do(){
        ...
        if($adminid==1){
            $data['shop_id']=1;
            $find=db('goods_cate')->where(['name'=>$data['name'],'shop_id'=>1])->find();
        }else{
            $data['shop_id']=$shop_id;
            $find=db('goods_cate')->where(['name'=>$data['name'],'shop_id'=>$shop_id])->find();
        }
        if($find){
            $this->error('分类名称已存在，请稍后更换！');
        }
        $do = db("goods_cate")->insert($data);
        if ($do) {
            $this->success("分类添加成功");
        } else {
    ...
    function cate_del(){
        ...
        foreach ($arr as $key => $value){
            $find=db('goods_cate')->where('id',$value)->find();
            if($find['fid']==0){
                $f=db('goods_cate')->where('fid',$value)->count();
                if($f>0){
                    $count++;
                }
            }
        ...
        $map['id'] = array("in", $id);
        $res=db("goods_cate")->where($map)->delete();
        if($res){
    ...
    function add(){
        ...
        $shop_id=$this->get_shop_id();
        if($adminid==1){
            $shop_list=db('shop')->where(['status'=>2,'id'=>1])->field('id,shop_name')->select();
        }else{
        ...
        if($adminid==1){
            $goods_cate=db('goods_cate')->where(['fid'=>0,'shop_id'=>1])->field('id,name')->select();
            // $goods_cate=db('goods_cate')->where('fid',0)->field('id,name')->select();
        }else{
            $goods_cate=db('goods_cate')->where(['fid'=>0,'shop_id'=>$shop_id])->field('id,name')->select();
        }
    ...
    function goods_cat(){
        ...
        $adminid = cookie("adminid");
        if($adminid==1){
            $info = db('goods_cate')->where('fid',$id)->field('id,name')->select();
        }else{
            $info = db('goods_cate')->where(['fid'=>$id,'shop_id'=>$shop_id])->field('id,name')->select();
        }
        $data = '';
        $data .=  '<option value="">请选择</option>';
        foreach ($info as $v){
    ...
    function edit(){
        ...
        $res=db('goods_size')->where('goods_id',$id)->select();
        ...
        if($adminid==1){
            $goods_cate=db('goods_cate')->where('fid',0)->field('id,name')->select();
            $goods_cate1=db('goods_cate')->where('id',$find['zsize_id'])->field('id,name')->find();
        }else{
            $goods_cate=db('goods_cate')->where(['fid'=>0,'shop_id'=>$shop_id])->field('id,name')->select();
            $goods_cate1=db('goods_cate')->where(['id'=>$find['zsize_id'],'shop_id'=>$shop_id])->field('id,name')->find();
        }
        $this->assign('shop_list',$shop_list);
        $this->assign('res',$res);
    ...
    function edit_do(){
        ...
        db('goods_size')->where('goods_id',$id)->delete();
